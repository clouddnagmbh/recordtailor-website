...
**Umgesetzt am 22. Juli 2026.** Kontaktformular auf `/kontakt` mit Server-Action `submitContact` (`app/kontakt/actions.ts`, `contact-form.tsx`): Honeypot-Feld `website` (versteckt) und Zeit-Falle `submittedAt` (Server verwirft Einreichungen unter 2 s Alter). Feldset: Name*, Firma, E-Mail*, aktuelles DMS-Select mit 10 Optionen, Nachricht*. Versand über die Resend-REST-API per `fetch` (`RESEND_API_KEY`-Env, `CONTACT_RECIPIENT_EMAIL`, `CONTACT_SENDER_EMAIL`); ohne Token fällt die Action in einen Dev-Log-Modus zurück. Cal.com-CTA als hervorgehobener Panel-Link neben dem Formular (Env `NEXT_PUBLIC_CAL_URL`). Datenschutz-Erklärung um Abschnitt „5. Kontaktformular" ergänzt: Zweckbindung Art. 6 Abs. 1 lit. b DSGVO, Nennung Resend als Auftragsverarbeiter, Hinweis auf Aufbewahrungspflichten. `public/llms-full.txt` als Route Handler unter `app/llms-full.txt/route.ts` — baut den Site-Inhalt beim Request aus allen strukturierten TS-Datenquellen (produkt, compare, migration, branchen, glossar, blog) als Markdown zusammen; damit automatisch synchron. `public/llms.txt` verweist am Ende darauf. `/glossar` mit 20 DE-Begriffen (GoBD, RAG, eIDAS, Legal Hold, IDP, Agentic AI, CMIS, ArchiveLink, Hybrid-Suche, Audit-Hash-Chain, Revisionssicher, IDW PS 880, EU AI Act, DSGVO, RLS, MCP, CAS, ISO 27001, On-Premise, Souveräne KI) — Übersichts-Kacheln + `/glossar/[slug]` mit `DefinedTerm`-JSON-LD + „Verwandte Begriffe". `/branchen` mit 4 Slugs (Steuerberater, öffentliche Verwaltung, Industrie & Fertigung, Gesundheit & Life Sciences) — jede Seite mit Lead, 4 Anforderungen, 4 Antworten, 3 Feature-Deep-Links, 3 FAQ und BreadcrumbList + FAQPage-JSON-LD. Roadmap-Themen (eAkte-BSI-TR-03138, GxP-Validierung, VS-NfD) ausdrücklich als Roadmap gekennzeichnet — keine falschen Behauptungen. Zusatzarbeiten aus Nutzer-Feedback in Phase 4: `/produkt` zeigt jetzt die 12 Fähigkeiten synchron zur Homepage (Sektion „Was drin steckt" mit denselben Kacheln), Feature-Titel „Acht Fähigkeiten…" → „Zwölf Fähigkeiten…" in DE + EN. `lib/compare-data.ts` refaktoriert auf `CompareCell` mit explizitem `status` (yes/partial/no) — die Tabelle zeigt Check/CircleDashed/MinusCircle statt pauschalem X; Feature-Liste pro Slug von 12–13 auf 30 Zeilen erweitert (Multi-Tenancy, Retention-Enforcement, Legal Hold, Air-Gap, SSO/MFA, Verschlüsselung, Undo, EU-AI-Act-Protokoll, Open-Source-Kern etc.). Legende unter jeder Tabelle plus Marken-Disclaimer. Footer um Vergleich/Branchen/Glossar-Links ergänzt. Build grün, Lint grün, EN-Routen und Middleware weiter intakt.
